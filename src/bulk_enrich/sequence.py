...
def validate_sequence(data: dict) -> None:
    ...
    for key in ("followups", "neutral_followups"):
        ...
        if (
            ...
            or (key == "followups" or templates)
            and set(templates) != set(FOLLOWUPS)
        ):
        ...
        for text in templates.values():
            ...
            if re.search(r"(?im)^p\.s\.", text) or "{{sender_name}}" in text:
                raise ValueError(
                    "follow-up templates must omit signature and P.S.; renderer appends them"
                )
    ps = seq.get("ps_variants", [])
    if not isinstance(ps, list) or any(
        not isinstance(x, str) or not x.strip() for x in ps
    ):
        raise ValueError("sequence.ps_variants must be an array of nonempty strings")
    for text in ps:
        if not text.startswith("p.s. ") or "\n" in text or "{{" in text:
            raise ValueError(
                "P.S. variants must be single literal lines beginning p.s. "
            )
    ...
    for key in ("editorial_replacements", "company_name_overrides"):
        ...
        for original, edit in edits.items():
            if (
                ...
                raise ValueError(
                    f"sequence.{key} requires text and reason for each exact replacement"
                )
    from bulk_enrich.config import forbidden_copy_character

    configured = [
        *seq.get("followups", {}).values(),
        *seq.get("neutral_followups", {}).values(),
        *ps,
    ]
    banned = data["personalization"].get("banned_phrases", [])
    claims = tuple(data["offer"].get("approved_claims", []))
    claims += (data["offer"].get("cta", ""), data["offer"].get("risk_reversal", ""))
    for text in configured:
        if forbidden_copy_character(text) or any(
            p.casefold() in text.casefold() for p in banned
        ):
            raise ValueError("sequence contains forbidden characters or banned wording")
        # Literal promises fail preflight; sentences containing slots are checked
        # after rendering against the selected approved commercial variants.
        for sentence in re.split(r"[.!?](?:\s+|$)|;\s*", text):
            if "{{" not in sentence and _unapproved_claim(sentence, claims):
                raise ValueError(
                    "sequence contains an unapproved figure or commercial promise"
                )
    if seq.get("greeting", "inline") == "inline":
        ...
        if not body.startswith(prefix):
            raise ValueError(
                "inline sequence greeting requires the standard Hi/pitch body prefix"
            )
...
def editorial_context(campaign: CampaignConfig, context: dict, domain: str) -> dict:
    ...
    for field in ("company_focus", "buyer_phrase"):
        ...
        result[field] = (
            seq.get("editorial_replacements", {})
            .get(original, {})
            .get("text", original)
        )
...
def inline_body(campaign: CampaignConfig, body: str) -> str:
    if (
        campaign.data.get("sequence", {}).get("greeting") == "paragraph"
        or "sequence" not in campaign.data
    ):
        return body
    match = re.match(r"^(Hi [^\n]+),\n\n(\S+)", body)
    if not match:
...
def render_sequence(
    campaign: CampaignConfig, context: dict, domain: str, first_body: str
) -> dict[str, str]:
    ...
    if any(not str(context.get(k, "")).strip() for k in required):
        ...
        if not templates:
            raise ValueError(
                "missing sequence slots and no approved neutral follow-ups"
            )
    ...
    claims += tuple(
        ...
        for value in (
            str(context.get("cta", "")),
            str(context.get("risk_reversal", "")),
        )
        for sentence in re.split(r"[.!?](?:\s+|$)|;\s*", value)
        if sentence.strip()
    )
    for index, (field, body) in enumerate(bodies.items()):
        ...
        limit = (
            ...
            if field == "personalized_email"
            else seq.get("max_followup_words", 55)
        )
        ...
        if forbidden_copy_character(body) or re.search(
            r"[\x00-\x08\x0b\x0c\x0e-\x1f\ufffd]", body
        ):
