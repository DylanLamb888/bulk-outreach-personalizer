...
class DeliveryTests(unittest.TestCase):
    def setUp(self):
        ...
        data["personalization"]["focus_rules_file"] = str(
        ...
        self.path = self.p / "campaign.json"
        ...
        self.output = self.p / "audit.csv"
        self.smart = self.p / "smartlead.csv"
    ...
    def run_source(self):
        return run_enrichment(
            ...
            commercial_focuses=CommercialFocusTable.load(
                self.campaign.focus_rules_path
            ),
            options=RunOptions(
                cache_dir=self.p / "cache", smartlead_output_path=self.smart
            ),
    ...
    def test_full_run_exports_only_complete_unique_sequences_and_offline_replays(self):
        ...
        with self.smart.open(encoding="utf-8-sig", newline="") as f:
        ...
        with (
            ...
            status = main(
                [
                    ...
                    str(self.output),
                    "--output",
                    str(self.p / "again.csv"),
                    "--campaign",
                    str(self.path),
                    "--render-only",
                    "--allow-test-campaign",
                    "--smartlead-output",
                    str(self.p / "again-smart.csv"),
                ]
            )
        self.assertEqual(status, 0)
        self.assertEqual(
            self.smart.read_bytes(), (self.p / "again-smart.csv").read_bytes()
        )

    def test_bad_followup_is_held_and_can_be_repaired_without_research(self):
        self.campaign.data["sequence"]["followups"]["followup_3b"] = (
            "We guarantee five qualified meetings every week."
        )
        report = self.run_source()
        self.assertEqual(report["delivery"]["included"], 0)
        result = run_render_only(
            input_path=self.output,
            output_path=self.p / "fixed.csv",
            campaign=load_campaign(self.path),
            options=RunOptions(
                cache_dir=self.p / "cache",
                smartlead_output_path=self.p / "fixed-smart.csv",
            ),
        )
    ...
    def test_replay_rejects_changed_audit_and_targeting_before_writing(self):
        ...
        for section, key, value in [
            ("offer", "audience", "different buyers"),
            ("personalization", "min_confidence", 0.9),
        ]:
            cfg = load_campaign(self.path)
            cfg.data[section][key] = value
            with self.assertRaisesRegex(ValueError, "classification inputs changed"):
                run_render_only(
                    input_path=self.output,
                    output_path=self.p / "rejected.csv",
                    campaign=cfg,
                    options=RunOptions(cache_dir=self.p / "cache"),
                )
        ...
        with patch(
            ...
            with self.assertRaisesRegex(ValueError, "distinct"):
                run_enrichment(
                    ...
                    options=RunOptions(
                        cache_dir=self.p / "cache", smartlead_output_path=self.input
                    ),
                )

    def test_ps_change_replays_without_duplicate_ps_or_changed_evidence(self):
        self.run_source()
        self.campaign.data["sequence"]["ps_variants"] = [
            'p.s. if you would prefer no more emails, reply "no thanks" and I will remove you.'
        ]
        result = run_render_only(
            input_path=self.output,
            output_path=self.p / "new.csv",
            campaign=self.campaign,
            options=RunOptions(
                cache_dir=self.p / "cache",
                smartlead_output_path=self.p / "new-smart.csv",
            ),
        )
        self.assertEqual(result["delivery"]["included"], 1)
        with (self.p / "new-smart.csv").open(encoding="utf-8-sig") as f:
            rows = list(csv.DictReader(f))
        self.assertEqual(rows[0]["personalized_email"].count("p.s."), 1)
        with self.output.open(encoding="utf-8-sig") as f:
            original = list(csv.DictReader(f))
        with (self.p / "new.csv").open(encoding="utf-8-sig") as f:
            revised = list(csv.DictReader(f))
        self.assertEqual(
            [r["company_fit_evidence"] for r in original],
            [r["company_fit_evidence"] for r in revised],
        )
